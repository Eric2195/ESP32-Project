...
static http_server_config_t *srv_cfg = NULL;

// ── HTML page (from original Arduino project) ─────────────────
...
function toast(msg,ok=true){const el=document.getElementById('toast');el.textContent=msg;
  el.style.borderColor=ok?'#28b878':'#c96a3e';el.classList.add('show');clearTimeout(tt);
...
function setBusy(b){isBusy=b;document.getElementById('busy').classList.toggle('show',b);
  ...
  document.querySelectorAll('.cbtn').forEach(el=>{if(el.id!=='blBtn')el.disabled=locked;});}
async function req(path){try{const r=await fetch(path);return r.ok;}catch(e){toast('no connection',false);return false;}}
async function waitNotBusy(){for(let i=0;i<100;i++){try{const r=await fetch('/state');
  const j=await r.json();if(!j.busy)return;}catch(e){}await new Promise(r=>setTimeout(r,150));}}
async function onBgChange(hex){if(canvasOpen){await req('/draw/clear?bg='+encodeURIComponent(hex));}
  else{await req('/redraw?bg='+encodeURIComponent(hex));}redrawCanvas(hex);}
...
async function setView(v){if(isBusy||termOpen||canvasOpen)return;if(v===3){toggleCanvas();return;}
  const keys=['w','s','d'];if(!await req('/cmd?k='+keys[v]))return;activeView=v;
  document.querySelectorAll('.vbtn').forEach(b=>b.classList.toggle('active',parseInt(b.dataset.v)===v));
  if(v===2){termOpen=true;document.getElementById('twrap').classList.add('open');
    setBusy(false);setBusy(false);
    document.querySelectorAll('.vbtn').forEach(b=>b.disabled=true);
    document.getElementById('tin').focus();toast('terminal open');return;}
  setBusy(true);await waitNotBusy();setBusy(false);}
async function toggleBL(){blOn=!blOn;await req('/backlight?on='+(blOn?1:0));
...
async function toggleCanvas(){canvasOpen=!canvasOpen;
  ...
  document.querySelectorAll('.vbtn').forEach(btn=>btn.classList.toggle('active',canvasOpen&&parseInt(btn.dataset.v)===3));
  await req('/canvas?on='+(canvasOpen?1:0));
  if(canvasOpen){const bg=document.getElementById('bgCol').value;redrawCanvas(bg);
    await req('/draw/clear?bg='+encodeURIComponent(bg));
    document.querySelectorAll('.vbtn').forEach(b=>b.disabled=true);toast('canvas active');
  }else{setBusy(false);toast('canvas off');}}
const tin=document.getElementById('tin');let lastVal='';
tin.addEventListener('input',async()=>{const cur=tin.value,prev=lastVal;
  if(cur.length>prev.length){await req('/char?c='+encodeURIComponent(cur[cur.length-1]));}
  else if(cur.length<prev.length){await req('/char?c=%08');}lastVal=cur;});
...
async function closeTerm(){await req('/cmd?k=q');termOpen=false;
  document.getElementById('twrap').classList.remove('open');setBusy(false);toast('terminal closed');}
const cvs=document.getElementById('cvs');const ctx=cvs.getContext('2d');let strokePts=[];
function getPos(e){const r=cvs.getBoundingClientRect();const sx=cvs.width/r.width,sy=cvs.height/r.height;
  const s=e.touches?e.touches[0]:e;return{x:(s.clientX-r.left)*sx,y:(s.clientY-r.top)*sy};}
function redrawCanvas(hex){ctx.fillStyle=hex;ctx.fillRect(0,0,cvs.width,cvs.height);}
function startDraw(e){e.preventDefault();drawing=true;strokePts=[];
...
function moveDraw(e){if(!drawing)return;e.preventDefault();const p=getPos(e);
  ctx.beginPath();ctx.moveTo(lastX,lastY);ctx.lineTo(p.x,p.y);
  ctx.strokeStyle=document.getElementById('penCol').value;
  ctx.lineWidth=4;ctx.lineCap='round';ctx.stroke();
  strokePts.push({x:Math.round(p.x),y:Math.round(p.y)});lastX=p.x;lastY=p.y;}
async function endDraw(e){if(!drawing)return;drawing=false;
  ...
  await req('/draw/stroke?pen='+pen+'&pts='+encodeURIComponent(pts));strokePts=[];}
cvs.addEventListener('mousedown',startDraw);cvs.addEventListener('mousemove',moveDraw);
...
async function clearAll(){const bg=document.getElementById('bgCol').value;
  redrawCanvas(bg);await req('/draw/clear?bg='+encodeURIComponent(bg));toast('cleared');}
(async()=>{try{const r=await fetch('/state');const j=await r.json();
  const spd=j.speed||1;document.getElementById('spd').value=spd;
  document.getElementById('spdV').textContent=spdLabels[spd];
  if(j.bl===false){blOn=false;const b=document.getElementById('blBtn');
    b.textContent='○ display off';b.classList.remove('on');b.classList.add('dim');}}catch(e){}
  document.getElementById('bgCol').value='#aa4818';redrawCanvas('#aa4818');})();
</script>
</body>
</html>
)rawhtml";
...
static esp_err_t route_root(httpd_req_t *req)
{
...
static esp_err_t route_cmd(httpd_req_t *req)
{
    char buf[8] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf));
    char val[4] = {0};
    if (httpd_query_key_value(buf, "k", val, sizeof(val)) != ESP_OK || strlen(val) == 0) {
        return httpd_resp_send(req, "{\"e\":1}", HTTPD_RESP_USE_STRLEN);
    }
    httpd_resp_send(req, "{\"ok\":1}", HTTPD_RESP_USE_STRLEN);
    if (srv_cfg && srv_cfg->on_cmd) srv_cfg->on_cmd(val[0]);
    return ESP_OK;
}

static esp_err_t route_char(httpd_req_t *req)
{
    char buf[16] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf));
    char val[8] = {0};
    if (httpd_query_key_value(buf, "c", val, sizeof(val)) == ESP_OK && strlen(val) > 0) {
        // Decode %XX
        char c = val[0];
        if (val[0] == '%' && strlen(val) >= 3) {
            c = (char)strtol(val + 1, NULL, 16);
        }
        if (srv_cfg && srv_cfg->on_char) srv_cfg->on_char(c);
    }
    return httpd_resp_send(req, "{\"ok\":1}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t route_speed(httpd_req_t *req)
{
    char buf[16] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf));
    char val[4] = {0};
    if (httpd_query_key_value(buf, "v", val, sizeof(val)) == ESP_OK) {
        ...
        if (v < 1) v = 1;
        if (v > 3) v = 3;
        if (srv_cfg && srv_cfg->on_speed) srv_cfg->on_speed(v);
    }
    return httpd_resp_send(req, "{\"ok\":1}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t route_backlight(httpd_req_t *req)
{
    char buf[16] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf));
    char val[4] = {0};
    ...
    if (srv_cfg && srv_cfg->on_backlight) srv_cfg->on_backlight(on);
    return httpd_resp_send(req, "{\"ok\":1}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t route_redraw(httpd_req_t *req)
{
    char buf[64] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf));
    char val[16] = {0};
    if (httpd_query_key_value(buf, "bg", val, sizeof(val)) == ESP_OK) {
        if (srv_cfg && srv_cfg->on_redraw) srv_cfg->on_redraw(val);
    }
    return httpd_resp_send(req, "{\"ok\":1}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t route_canvas(httpd_req_t *req)
{
    char buf[16] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf));
    char val[4] = {0};
    ...
    if (srv_cfg && srv_cfg->on_canvas) srv_cfg->on_canvas(on);
    return httpd_resp_send(req, "{\"ok\":1}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t route_draw_clear(httpd_req_t *req)
{
    char buf[64] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf));
    char val[16] = {0};
    if (httpd_query_key_value(buf, "bg", val, sizeof(val)) == ESP_OK) {
        if (srv_cfg && srv_cfg->on_draw_clear) srv_cfg->on_draw_clear(val);
    }
    return httpd_resp_send(req, "{\"ok\":1}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t route_draw_stroke(httpd_req_t *req)
{
    char buf[4096] = {0};
    httpd_req_get_url_query_str(req, buf, sizeof(buf));
    char pen[16] = {0};
    char pts[3800] = {0};
    httpd_query_key_value(buf, "pen", pen, sizeof(pen));
    httpd_query_key_value(buf, "pts", pts, sizeof(pts));
    if (srv_cfg && srv_cfg->on_draw_stroke) srv_cfg->on_draw_stroke(pen, pts);
    return httpd_resp_send(req, "{\"ok\":1}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t route_state(httpd_req_t *req)
{
    char j[128];
    snprintf(j, sizeof(j), "{\"view\":%d,\"busy\":%s,\"term\":%s,\"bl\":%s,\"speed\":%d}",
             srv_cfg ? *srv_cfg->current_view : 0,
             (srv_cfg && *srv_cfg->busy) ? "true" : "false",
             (srv_cfg && *srv_cfg->term_mode) ? "true" : "false",
             (srv_cfg && *srv_cfg->backlight_on) ? "true" : "false",
             srv_cfg ? *srv_cfg->anim_speed : 1);
...
esp_err_t http_server_start(http_server_config_t *cfg)
{
    ...
    httpd_uri_t routes[] = {
        { .uri = "/",            .method = HTTP_GET, .handler = route_root },
        { .uri = "/cmd",         .method = HTTP_GET, .handler = route_cmd },
        { .uri = "/char",        .method = HTTP_GET, .handler = route_char },
        { .uri = "/speed",       .method = HTTP_GET, .handler = route_speed },
        { .uri = "/backlight",   .method = HTTP_GET, .handler = route_backlight },
        { .uri = "/redraw",      .method = HTTP_GET, .handler = route_redraw },
        { .uri = "/canvas",      .method = HTTP_GET, .handler = route_canvas },
        { .uri = "/draw/clear",  .method = HTTP_GET, .handler = route_draw_clear },
        { .uri = "/draw/stroke", .method = HTTP_GET, .handler = route_draw_stroke },
        { .uri = "/state",       .method = HTTP_GET, .handler = route_state },
    };
